...
function saveSettings() {
    ...
    // Save selected prompt name
    localStorage.setItem('selectedPromptName', promptSelectInput.value);
...
function loadSettings() {
    ...
    if (savedJobType) {
        const radio = document.getElementById(`${savedJobType}Radio`);
        if (radio) radio.checked = true;
    }

    // Load prompt name, but only display it. The full prompt object will be loaded in initializePrompts
    promptSelectInput.value = localStorage.getItem('selectedPromptName') || '';
    ...
    // Call this here to ensure the UI elements are correctly shown/hidden on load
