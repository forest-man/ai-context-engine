 ...
 * @param {string} jsonData The stringified JSON content.
 * @returns {Promise<string>} The URI of the uploaded file.
 */
function uploadFileToGemini(geminiKey, filename, jsonData) {
    ...
    const response = fetch(uploadUrl, { method: 'POST', body: formData })
        .then(response => {
            if (!response.ok) {
 ...
 * @param {string} geminiKey The Gemini API key.
 * @param {string} fileUri The URI of the file to delete (e.g., 'files/abc123xyz').
 * @returns {Promise<void>}
 */
async function deleteFileFromGemini(geminiKey, fileUri) {
    if (!fileUri || !geminiKey) return;

    // fileUri format is typically 'files/id'. We need to extract the ID if needed, 
    // but the DELETE endpoint usually accepts the full resource name.
    const deleteUrl = `https://generativelanguage.googleapis.com/v1beta/${fileUri}?key=${geminiKey}`;
    ...
    try {
        ...
        if (!response.ok) {
            // Note: The API often returns 404/400 for already deleted files, which we can ignore.
            if (response.status !== 404) {
                const errorData = await response.json();
                console.warn(`File API deletion failed for ${fileUri}. Status: ${response.status}. Error: ${errorData.error.message}`);
            }
        } else {
            console.log(`File deleted successfully: ${fileUri}`);
        }
    } catch (error) {
        console.warn(`Error during file deletion for ${fileUri}:`, error);
    }
}
 ...
 * @param {string} fsDomain The FreshService domain for link construction inside the prompt.
 * @param {string} fileUri The URI of the pre-uploaded file.
 */
async function analyzeOverallWithGemini(geminiKey, selectedModel, promptData, fsDomain, fileUri) {
    const rateLimitDelay = parseInt(rateLimitDelayInput.value, 10) * 1000 || 60000;
    ...
    if (!fileUri) {
        // This should not happen if called correctly from main.js, but as a safeguard:
        throw new Error("File URI is missing. Data must be uploaded before calling analyzeOverallWithGemini.");
    }
    ...
    // CRITICAL FIX: Ensure FreshService domain is correctly sanitized and included in the prompt
    const sanitizedDomain = fsDomain.replace(/^https?:\/\//, '');

    finalPrompt = finalPrompt.replace(/\[FILENAME_HERE\]/g, filename);
    finalPrompt = finalPrompt.replace(/\[REPORT_NAME\]/g, "DARYL-Report"); // Placeholder used in some prompts
    finalPrompt = finalPrompt.replace(/\[FS_DOMAIN_HERE\]/g, sanitizedDomain);
    // CRITICAL LINK FIX: This ensures the mandatory link structure required by the prompt files uses the domain
    finalPrompt = finalPrompt.replace(/cbportal\.freshservice\.com/g, sanitizedDomain);
    ...
    const payload = {
        "contents": [{
            "parts": [{
                ...
                "fileData": {
                    "mimeType": "text/plain",
                    "fileUri": fileUri // Use the pre-uploaded URI
                }
            }]
        }],
        "systemInstruction": {
 ...
 * This function now uses the pre-uploaded file URI.
 * @param {string} geminiKey The Gemini API key.
 * @param {string} selectedModel The Gemini model to use.
 * @param {object} promptData The prompt data object.
 * @param {string} filename The filename for the uploaded data (for prompt context).
...
async function countOverallTokensWithGemini(geminiKey, selectedModel, promptData, filename, fileUri) {
    if (!fileUri) throw new Error("File URI required for token count.");
    ...
    // Prepare prompt content with placeholders replaced (using fsDomainInput directly for now)
    const finalPrompt = promptData.prompt
        .replace(/\[FILENAME_HERE\]/g, filename)
        .replace(/\[FS_DOMAIN_HERE\]/g, sanitizedDomain)
        .replace(/cbportal\.freshservice\.com/g, sanitizedDomain); // CRITICAL FIX: Ensure prompt links are updated for token count context
